...
Адаптер написан и покрыт юнит-тестами, но в работающем control-plane он пока не создаётся
ни одним модулем: ни `server.ts`, ни `configured-lifecycle.ts` его не конструируют. Переменные
выше валидируются при старте, а сам клиент остаётся неиспользуемым. Точка подключения
закреплена за F6 — см. C5 в `docs/plans/actual_plan.md`.
