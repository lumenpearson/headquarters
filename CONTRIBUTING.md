...
- Набор `postgres.integration.test.ts` разрушающий: он создаёт и удаляет базы `hqtest_*`
  рядом с той, что названа в URL, подметает брошенные старше часа перед стартом и
  отказывается запускаться, если URL разрешается в ту же базу, что
  `HQ_CONTROL_PLANE_DATABASE_URL`. Направляйте его только на отдельную базу — `hq_scratch`,
  не на боевую.
