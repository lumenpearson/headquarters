...
describe('setting scope localization', () => {
  ...
  it('falls back to the bare scope word uppercased before the catalogue carries an entry', () => {
    // `localizedSettingScope` reads through `messages.ts`'s merged catalogue,
    // which does not yet carry `settingLabelMessages.ts` (`catalog/index.ts`
    // is out of this pass's scope -- see the module doc). Until it is wired
    // in, the fallback below is what an operator sees; `settingLabelMessages`
    // above is what they will see once it is.
    expect(localizedSettingScope('device', 'ru')).toBe('DEVICE');
    expect(localizedSettingScope('group', 'ru')).toBe('GROUP');
  });
});
