      ...
      <div className="system-layout">
        ...
        >
          <div className="metric-grid metric-grid--four">
            ...
            />
            <Metric label="GPU" value={`${state.metrics.gpu}%`} detail="VIDEO PIPELINE" tone="ok" />
            <Metric
              label="STORAGE"
        ...
        <Panel title="СЕТЕВЫЕ КАНАЛЫ" eyebrow="ENCRYPTED LINKS" className="system-network">
          {channels.slice(0, 7).map((channel) => (
            ...
            >
              ...
              <ProgressBar value={channel.signal} tone={channel.signal < 50 ? 'critical' : 'ok'} />
            </TerminalButton>
          ))}
        </Panel>
        ...
        >
          <div className="audit-log">
            {state.audit.slice(0, 14).map((entry) => (
              <div key={entry.id}>
        ...
        <Panel title="КОНТУР ХРАНЕНИЯ" eyebrow="LOCAL / OFFLINE" className="system-storage">
          <div className="storage-map">
            {['CORE', 'EVENTS', 'VIDEO', 'EVIDENCE', 'ARCHIVE', 'SNAPSHOTS'].map((item, index) => (
              <div key={item}>
                <i>[{String(index + 1).padStart(2, '0')}]</i>
                <span>{item}</span>
                <b>{[48, 63, 82, 57, 74, 12][index]}%</b>
              </div>
            ))}
          </div>
