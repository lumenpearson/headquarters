...
const failures: Readonly<Record<ControlPlaneFailureCode, ControlPlaneFailureShape>> = {
  ...
  [ControlPlaneFailure.INTEGRATION_GITHUB_UNAVAILABLE]: {
    // `failed-precondition`, not `unimplemented`: the method exists and the
    // build serves it, and what is missing is configuration this deployment
    // can add. `unimplemented` tells an operator the opposite -- that the
    // plane will never answer -- and sends them looking for a different
    // build. `integration/service.test.ts` holds this distinction.
    status: Code.FailedPrecondition,
