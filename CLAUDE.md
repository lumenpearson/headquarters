...
- `localStorage` owns everything the browser persists, under eight keys:
  ...
  cloud state, staged under `…:draft` first), `hq.camera-material-assignments.v1`,
  `hq.keybinds-intro-seen.v1`, and the Yandex Maps key. No IndexedDB, no Tauri store plugin.
